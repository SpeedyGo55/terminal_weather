...
use tui_input::Input;
use ratatui::widgets::{Axis, Chart, Dataset, GraphType, Wrap};
...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        terminal.draw(|frame| {
            ...
            let mut forecast_rain_list: &mut [(f64, f64); 40] = &mut [(0.0, 0.0); 40];
            
            forecast_rain_list.iter_mut().set_from(forecast_rain_vec.iter().cloned());
            
            let mut forecast_rain_dataset = Dataset::default()
                .name("Rain")
                .marker(symbols::Marker::Braille)
                .graph_type(GraphType::Line)
